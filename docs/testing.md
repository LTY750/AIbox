...
There is currently no `test/e2e` Playwright project. `pnpm run test:e2e` is kept
as a compatibility alias and runs the available Vitest integration suite under
`test/integration`; a future browser runner should update this script and this
document together.
