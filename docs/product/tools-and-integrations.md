...
AIbox Mobile 通过工具系统扩展 AI 的能力，使 AI 不仅能生成文本，还能执行搜索、读取文件、查询知识库、调用第三方服务等操作。
...
使用方式：在会话中开启"网页浏览"模式，AI 在需要搜索或读取网页时会自动调用对应工具。如果当前提供方不支持读取网页（例如 BoCha），AI 不会被注入 `parse_link` 工具，避免出现"调用了但失败"的情况。

### 工具失败时的引导

当工具因为缺少配置而失败（例如选择了 Tavily 但没有填写 API Key），消息中的工具气泡会自动展开，显示本地化的错误说明，并提供一键跳转链接前往设置页，不再只是"工具调用失败"的红色提示。
...
桌面端为兼容 Chatbox AI 账户预置了一组云端 MCP 服务（付费功能），无需配置即可使用；Android 仅支持用户配置的远程服务：
...
用户可以添加自己的 MCP 服务器，扩展 AI 的能力边界。应用只连接远程服务：
