...
describe('Sentry event policy', () => {
  ...
  test('redacts explicit private fields and credentials while preserving diagnostic fields', () => {
    const event = {
      ...
      request: {
        ...
        url: 'https://alice:secret@example.com/path?token=secret#section',
      },
  ...
  test('redacts URL userinfo in free-form error text', () => {
    const event = {
      exception: { values: [{ value: 'request failed at https://alice:secret@example.com/private' }] },
      tags: { component: 'network', operation: 'request' },
    }

    expect(createProcessor()(event)).toBe(event)
    expect(event.exception.values[0].value).toBe('request failed at https://[redacted]@example.com/private')
  })
})
