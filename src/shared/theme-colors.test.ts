...
describe('INTERFACE_COLOR_PRESETS', () => {
  ...
  it('uses the application default colors for the default preset', () => {
    ...
    expect(INTERFACE_COLOR_PRESETS[0].fontFamily).toBe(DEFAULT_INTERFACE_FONT_FAMILY)
  })

  it('applies a Claude-style font to the Claude preset', () => {
    expect(INTERFACE_COLOR_PRESETS[1].fontFamily).toBe('claude')
    expect(INTERFACE_FONT_FAMILY_CSS.claude).toContain('Lora Variable')
    expect(INTERFACE_FONT_FAMILY_CSS.claude).toContain('Noto Serif SC')
    expect(INTERFACE_FONT_FAMILY_CSS.claude).toContain('SimSun')
  })
