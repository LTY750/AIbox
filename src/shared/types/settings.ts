...
const ExtensionSettingsSchema = z.object({
  webSearch: z.object({
    // Only supported third-party search services are exposed. Legacy provider
    // values are normalized to Tavily by the catch fallback during migration.
    provider: z.enum(['tavily', 'bocha']).catch('tavily'),
...
const InterfaceColorPresetSchema = z.object({
  ...
  fontFamily: z.enum(['system', 'claude', 'serif', 'mono']).optional(),
})
...
export const SettingsSchema = GlobalSessionSettingsSchema.extend({
  ...
  interfaceFontFamily: z.enum(['system', 'claude', 'serif', 'mono']).catch(DEFAULT_INTERFACE_FONT_FAMILY),
  language: z.enum([
    'en',
    'zh-Hans',
    'zh-Hant',
