...
function applyCompaction(
  ...
  // Keep persisted summaries in fallback contexts as well. Older mobile
  // sessions can contain a summary without a matching compaction-point row;
  // dropping it here silently loses the compressed representation.
...
function applyMessageLimit(messages: Message[], maxCount: number): Message[] {
  ...
  const summary = workingMsgs.find((message) => message.isSummary)
  const history = workingMsgs.filter((message) => !message.isSummary)

  // maxCount limits history, +1 for the current input (last message)
  const effectiveLimit = Math.max(0, maxCount) + 1

  // A compaction summary is the replacement for the discarded prefix and must
  // never be removed by the independent message-count limit.
  const result = history.slice(-effectiveLimit).map((m) => ({ ...m }))
  if (summary) {
