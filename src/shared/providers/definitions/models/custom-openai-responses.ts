...
export default class CustomOpenAIResponses extends AbstractAISDKModel {
    ...
    return fetchRemoteModels(
      {
        ...
        apiKey: this.options.apiKey,
        // The Android WebView cannot access many OpenAI-compatible /models
        // endpoints because their CORS headers are incomplete. Use native HTTP
        // on mobile, independently of the desktop compatibility switch.
        useProxy: this.options.useProxy === true || this.dependencies.platformType === 'mobile',
      },
      this.dependencies
    )
  }
  ...
  public async paint(
    ...
    // Images APIs are separate from the Responses API. Reuse the compatible
    // transport so a single provider can serve Responses chat and image models.
    const imageProvider = new CustomOpenAI(
      {
        ...this.options,
    ...
    return imageProvider.paint(params, signal, callback)
  }
