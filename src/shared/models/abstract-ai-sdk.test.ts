...
import AbstractAISDKModel, {
  DEFAULT_MAX_MODEL_STEPS,
  isRetryableStatusError,
  MAX_MODEL_STEPS,
  resolveMaxModelSteps,
} from './abstract-ai-sdk'
...
describe('max model steps', () => {
  it('uses a finite default and clamps invalid or excessive values', () => {
    expect(resolveMaxModelSteps(undefined)).toBe(DEFAULT_MAX_MODEL_STEPS)
    expect(resolveMaxModelSteps(0)).toBe(DEFAULT_MAX_MODEL_STEPS)
    expect(resolveMaxModelSteps(Number.NaN)).toBe(DEFAULT_MAX_MODEL_STEPS)
    expect(resolveMaxModelSteps(3.8)).toBe(3)
    expect(resolveMaxModelSteps(MAX_MODEL_STEPS + 1)).toBe(MAX_MODEL_STEPS)
  })
})
