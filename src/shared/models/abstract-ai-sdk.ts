...
/** Prevent an omitted or malformed maxSteps value from creating an unbounded tool loop. */
export const DEFAULT_MAX_MODEL_STEPS = 32
export const MAX_MODEL_STEPS = 64

export function resolveMaxModelSteps(value: number | undefined): number {
  if (!Number.isFinite(value) || value === undefined || value <= 0) {
    return DEFAULT_MAX_MODEL_STEPS
  }
  return Math.min(MAX_MODEL_STEPS, Math.max(1, Math.floor(value)))
}
...
export default abstract class AbstractAISDKModel implements ModelInterface {
  ...
  public async *chatStream<T extends ToolSet>(
    ...
    const result = streamText({
      model,
      messages,
      stopWhen: [stepCountIs(resolveMaxModelSteps(options.maxSteps)), stopWhenPersistentToolCallPause<T>()],
