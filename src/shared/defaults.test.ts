...
import { DEFAULT_INTERFACE_COLORS, DEFAULT_INTERFACE_FONT_FAMILY } from './theme-colors'
...
describe('defaults', () => {
  it('settings() returns expected default values', () => {
    ...
    expect(result.interfaceFontFamily).toBe(DEFAULT_INTERFACE_FONT_FAMILY)
  ...
  it('settings() keeps telemetry disabled until the user consents', () => {
  ...
  it('uses the system font for legacy settings', () => {
    const legacySettings = { ...settings(), interfaceFontFamily: undefined }

    expect(SettingsSchema.parse(legacySettings).interfaceFontFamily).toBe(DEFAULT_INTERFACE_FONT_FAMILY)
  })
