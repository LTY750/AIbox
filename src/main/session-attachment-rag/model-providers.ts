...
export async function getSessionAttachmentEmbeddingProviderWithResolution(): Promise<SessionAttachmentEmbeddingProviderResolution> {
  ...
  try {
    const provider = await createEmbeddingProviderFromModelString(defaultEmbeddingModel)
    return {
      provider,
      modelString: defaultEmbeddingModel,
      source: 'default-embedding-model',
    }
  } catch (error) {
    log.error(`[MODEL] Failed to resolve session attachment embedding provider: ${defaultEmbeddingModel}`, error)
    sentry.withScope((scope) => {
      scope.setTag('component', 'session-attachment-rag-model')
      scope.setTag('operation', 'get_embedding_provider')
      scope.setExtra('embeddingModel', defaultEmbeddingModel)
