...
import { MineruParser, TextInParser, testMineruConnection } from './parsers'
...
const activeTextInParseTasks = new Map<string, AbortController>()
...
export function registerKnowledgeBaseHandlers() {
  ...
  // Parse file with TextIn XParse (for InputBox file attachments)
  ipcMain.handle(
    'parser:parse-file-with-textin',
    async (
      _event,
      params: {
        filePath: string
      ...
      const { filePath, filename, mimeType, appId, secretCode } = params

      try {
        log.info(`ipcMain: parser:parse-file-with-textin, filename=${filename}, mimeType=${mimeType}`)

        if (!filePath || !filePath.trim()) {
          return { success: false, error: 'File path is required' }
        }
        if (!appId?.trim() || !secretCode?.trim()) {
          return { success: false, error: 'textin_credentials_required' }
        }

        const abortController = new AbortController()
        activeTextInParseTasks.set(filePath, abortController)

        try {
          const parser = new TextInParser(appId, secretCode)
          const content = await parser.parse(
            filePath,
            {
              fileId: Date.now(),
          ...
          log.info(`ipcMain: parser:parse-file-with-textin completed, content length=${content.length}`)
          return { success: true, content }
        } finally {
          activeTextInParseTasks.delete(filePath)
        }
      } catch (error: unknown) {
        if (error instanceof Error && error.name === 'AbortError') {
          log.info(`ipcMain: parser:parse-file-with-textin cancelled, filename=${filename}`)
          return { success: false, cancelled: true, error: 'Operation cancelled' }
        }

        log.error('ipcMain: parser:parse-file-with-textin failed', error)
        sentry.withScope((scope) => {
          scope.setTag('component', 'knowledge-base-ipc')
          scope.setTag('operation', 'parse_file_with_textin')
          scope.setExtra('filename', params?.filename)
          scope.setExtra('mimeType', params?.mimeType)
          sentry.captureException(error)
        })
  ...
  ipcMain.handle('parser:cancel-textin-parse', (_event, filePath: string) => {
    try {
      const controller = activeTextInParseTasks.get(filePath)
      if (controller) {
        controller.abort()
        activeTextInParseTasks.delete(filePath)
      }
      ...
      log.error('ipcMain: parser:cancel-textin-parse failed', error)
      return { success: false, error: error instanceof Error ? error.message : String(error) }
    }
  })
}
