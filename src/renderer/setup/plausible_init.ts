...
export async function initPlausibleTracking(subscribeToNavigation?: PlausibleNavigationSubscriber): Promise<void> {
  try {
    ...
    if (settings.allowReportingAndTracking && !window.plausible) {
      ...
      script.defer = true
      script.dataset.domain = CHATBOX_BUILD_PLATFORM === 'web' ? 'web.chatboxai.app' : 'app.chatboxai.app'
      script.src = 'https://plausible.midway.run/js/script.local.hash.manual.js'
