...
function Root() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // Remote config and the model registry use cached/snapshot data for the
      // first paint. Refresh them after the shell is interactive.
      const refreshRemoteData = () => {
        void prefetchModelRegistry()
        void remote
          .getRemoteConfig('current_version')
          .then((remoteConfig) => setRemoteConfig(async (prev) => ({ ...(await prev), ...remoteConfig })))
          .catch(() => undefined)
      }
      if (typeof window.requestIdleCallback === 'function') {
        window.requestIdleCallback(refreshRemoteData, { timeout: 3000 })
      } else {
        window.setTimeout(refreshRemoteData, 1500)
      }
      ...
      // On store builds, the guide guard is evaluated again when deferred remote
      // config arrives; it must not hold the first screen hostage.
      const isStoreReviewPlatform =
        CHATBOX_BUILD_PLATFORM === 'ios' ||
        (CHATBOX_BUILD_PLATFORM === 'android' && CHATBOX_BUILD_CHANNEL === 'google_play')
      if (isStoreReviewPlatform && !isExceededResolved) return

      initialized.current = true

      // The legacy Chatbox AI onboarding and startup promotion are disabled.
