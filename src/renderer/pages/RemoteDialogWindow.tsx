...
export default function RemoteDialogWindow() {
  ...
  useEffect(() => {
    const initialCheck = window.setTimeout(() => void checkRemoteDialog(), 2500)
    const interval = window.setInterval(() => void checkRemoteDialog(), 1000 * 60 * 60 * 24) // 对于常年不关机的用户，也要每天检查一次
    return () => {
