...
export function cancelGeneration(): void {
  ...
  if (store.currentGeneratingId) {
    // Abort the in-flight provider request
...
export async function resumeGeneration(recordId: string): Promise<ImageGeneration | null> {
  ...
  // Direct provider requests do not expose a resumable remote task. Retry the
  // original prompt through the provider registry instead of contacting any
  // legacy remote task endpoint.
  await retryGeneration(recordId)
  return await platform.getImageGenerationStorage().getById(recordId)
}
