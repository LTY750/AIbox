...
} from '@/native/background-stream-recovery'
...
type MobileGenerationLifecycleEntry = {
  flush: () => Promise<void>
  cancelGeneration: () => void
  backgrounded: boolean
}

const mobileGenerationLifecycleEntries = new Map<string, MobileGenerationLifecycleEntry>()
let mobileGenerationLifecycleSubscriptionInstalled = false
let removeMobileGenerationLifecycleSubscription: (() => void) | undefined
let removeMobileGenerationVisibilitySubscription: (() => void) | undefined
let mobileGenerationCancellationSubscriptionInstalled = false
let removeMobileGenerationCancellationSubscription: (() => void) | undefined

function mobileGenerationLifecycleKey(sessionId: string, messageId: string): string {
  return `${sessionId}:${messageId}`
}

function ensureMobileGenerationLifecycleSubscription(): void {
  if (platform.type !== 'mobile') {
    return
  }

  if (!mobileGenerationCancellationSubscriptionInstalled) {
    mobileGenerationCancellationSubscriptionInstalled = true
    removeMobileGenerationCancellationSubscription = registerNativeStreamCancellationHandler(() => {
      // The foreground notification's Stop command intentionally cancels all
      // native requests, so each active generation must receive the same abort.
      for (const entry of mobileGenerationLifecycleEntries.values()) {
        entry.cancelGeneration()
      }
    })
  }

  if (mobileGenerationLifecycleSubscriptionInstalled) {
    return
  }
  if (!platform.onAppStateChange) {
    return
  }
  const handleMobileAppState = ({ isActive }: { isActive: boolean }) => {
    if (!isActive) {
      for (const entry of mobileGenerationLifecycleEntries.values()) {
        entry.backgrounded = true
        // The native foreground service owns the request while the WebView is
        // paused. Persist what the renderer has already processed so a later
        // recovery never replaces visible partial output with an error state.
        void entry.flush().catch((error) => {
          console.warn('Failed to persist background stream progress:', error)
        })
      }
      return
    }

    const entries = [...mobileGenerationLifecycleEntries.values()].filter((entry) => entry.backgrounded)
    if (entries.length === 0) {
      return
    }

    void recoverNativeStreamsAfterForeground()
      .then(() => {
        // The stream bridge reattaches first and replays missed chunks before
        // clearing this flag. A real native terminal error still flows through
        // the existing generation error handling below.
        for (const entry of entries) {
          entry.backgrounded = false
        }
      })
      .catch((error) => {
        // Keep entries marked as backgrounded so a later active transition can
        // retry attachment. Do not turn partial output into a renderer error:
        // the native task may still be running.
        console.warn('Failed to recover native streams after foregrounding:', error)
      })
  }
  mobileGenerationLifecycleSubscriptionInstalled = true
  removeMobileGenerationLifecycleSubscription = platform.onAppStateChange(handleMobileAppState)

  // Capacitor normally emits appStateChange, but visibilitychange is a useful
  // WebView fallback when the OS suspends the page before the native callback.
  if (typeof document !== 'undefined') {
    const onVisibilityChange = () => handleMobileAppState({ isActive: document.visibilityState === 'visible' })
    document.addEventListener('visibilitychange', onVisibilityChange)
    removeMobileGenerationVisibilitySubscription = () => {
      document.removeEventListener('visibilitychange', onVisibilityChange)
      removeMobileGenerationVisibilitySubscription = undefined
    }
  }
}

function registerMobileGenerationLifecycle(
  sessionId: string,
  messageId: string,
  persistCurrentMessage: () => Promise<void>,
  cancelGeneration: () => void
): () => void {
  if (platform.type !== 'mobile') {
    return () => {}
  ...
  ensureMobileGenerationLifecycleSubscription()
  const key = mobileGenerationLifecycleKey(sessionId, messageId)
  let flushInFlight: Promise<void> | undefined
  const flush = () => {
    const next = (flushInFlight ?? Promise.resolve()).then(persistCurrentMessage)
    const trackedFlush = next.then(
      () => undefined,
      () => undefined
    )
    flushInFlight = trackedFlush
    void trackedFlush.finally(() => {
  ...
  const entry: MobileGenerationLifecycleEntry = {
    flush,
    cancelGeneration,
    backgrounded: false,
  }
  mobileGenerationLifecycleEntries.set(key, entry)
  return () => {
    if (mobileGenerationLifecycleEntries.get(key) === entry) {
      mobileGenerationLifecycleEntries.delete(key)
    }
  }
}

export function resetMobileGenerationLifecycleForTests(): void {
  mobileGenerationLifecycleEntries.clear()
  removeMobileGenerationLifecycleSubscription?.()
  removeMobileGenerationLifecycleSubscription = undefined
  mobileGenerationLifecycleSubscriptionInstalled = false
  removeMobileGenerationVisibilitySubscription?.()
  removeMobileGenerationVisibilitySubscription = undefined
  removeMobileGenerationCancellationSubscription?.()
  removeMobileGenerationCancellationSubscription = undefined
  mobileGenerationCancellationSubscriptionInstalled = false
}
...
export async function orchestrateGeneration(
  ...
  const unregisterMobileGenerationLifecycle = registerMobileGenerationLifecycle(
    sessionId,
    targetMsg.id,
    () => persistStreamingMessage(sessionId, targetMsg),
    () => controller.abort(Date.now())
  )
  ...
  try {
    ...
    unregisterMobileGenerationLifecycle()
  }
}
