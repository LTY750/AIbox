...
function canUseSessionAttachmentRag(): boolean {
  ...
  log.debug(
    `${SESSION_ATTACHMENT_RAG_LOG_PREFIX} Capability skipped: no default embedding model, platform=${platform.type}`
  )
...
async function fallbackToLlamaParser(
  file: File,
  uniqKey: string,
  reason: 'local_parser_failed' | 'empty_content'
): Promise<{ content: string; storageKey: string; tokenCountMap: Record<string, number>; parserType: string }> {
  log.warn(`Falling back to LlamaParse for "${file.name}" due to ${reason}`)

  try {
    return await parseFileWithLlamaParse(file, uniqKey)
  } catch (error) {
    log.error(`LlamaParse fallback parsing failed for "${file.name}":`, error)
    ...
    if (error instanceof Error && error.message === 'llama_parse_api_key_required') {
      throw error
    }
...
type LocalParserFallbackOptions = {
  allowLlamaFallback?: boolean
  forceLlamaFallback?: boolean
}

function shouldFallbackToLlama(options: LocalParserFallbackOptions): boolean {
  return Boolean(options.forceLlamaFallback) || options.allowLlamaFallback !== false
}
...
async function parseFileWithLocalFallback(
  ...
  try {
    ...
    if (!hasParsedText(result.content)) {
      if (shouldFallbackToLlama(options)) {
        return await fallbackToLlamaParser(file, uniqKey, 'empty_content')
      }
    ...
    if (shouldFallbackToLlama(options)) {
      return await fallbackToLlamaParser(file, uniqKey, 'local_parser_failed')
    }
...
/**
 * Parse file using LlamaParse cloud service
 */
async function parseFileWithLlamaParse(
  file: File,
  uniqKey: string
): Promise<{ content: string; storageKey: string; tokenCountMap: Record<string, number>; parserType: string }> {
  const parserConfig = getEffectiveDocumentParserConfig()
  const content = await parseLlamaParseApi(file, { apiKey: parserConfig.llamaParse?.apiKey || '' })
  ...
  return { content, storageKey: uniqKey, tokenCountMap: {}, parserType: 'llamaparse' }
}
...
/**
 * Parse file using TextIn XParse (Desktop only)
 */
async function parseFileWithTextinService(
  file: File,
  uniqKey: string,
  credentials: { appId: string; secretCode: string }
): Promise<{ content: string; storageKey: string; tokenCountMap: Record<string, number>; parserType: string }> {
  if (!platform.parseFileWithTextin) {
    throw new Error('third_party_parser_not_supported_in_chat')
  }

  const result = await platform.parseFileWithTextin(file, credentials)
  if (result.cancelled) {
    throw new Error('parsing_cancelled')
  }
  if (!result.success) {
    throw new Error(result.error || 'third_party_parser_failed')
  }
  if (!result.content || !hasParsedText(result.content)) {
    throw new Error(EMPTY_ATTACHMENT_CONTENT_ERROR)
  }
  ...
  try {
    ...
    if (isTextFilePath(file.name)) {
      ...
      result = await parseFileWithLocalFallback(file, uniqKey, {
        allowLlamaFallback: options?.source !== 'pasted-text',
      })
    } else {
      const parserConfig = getEffectiveDocumentParserConfig()
      ...
      switch (parserConfig.type) {
        ...
        case 'llamaparse': {
          result = await parseFileWithLocalFallback(file, uniqKey, { forceLlamaFallback: true })
          break
        }
        ...
        case 'textin': {
          const appId = parserConfig.textin?.appId
          const secretCode = parserConfig.textin?.secretCode
          if (!appId?.trim() || !secretCode?.trim()) {
            throw new Error('textin_credentials_required')
          }
          try {
            result = await parseFileWithTextinService(file, uniqKey, { appId, secretCode })
          } catch (error) {
            log.error(`TextIn parsing failed for "${file.name}":`, error)
            if (
              error instanceof Error &&
              (error.message === EMPTY_ATTACHMENT_CONTENT_ERROR ||
                error.message === 'parsing_cancelled' ||
...
export async function preprocessLink(
  ...
  try {
    ...
    // Always parse links locally. Web search providers (Tavily/Bocha) own
    // network search; Chatbox-hosted Pro parsing is intentionally unavailable.
