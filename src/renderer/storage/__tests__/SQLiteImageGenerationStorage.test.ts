...
import { resetSharedSQLiteConnectionsForTests } from '../sqliteConnection'
...
describe('SQLiteImageGenerationStorage', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    resetSharedSQLiteConnectionsForTests()
    mockConnection.isConnection.mockResolvedValue({ result: false })
  ...
  it('shares one native connection across concurrent storage initialization', async () => {
    const first = new SQLiteImageGenerationStorage()
    const second = new SQLiteImageGenerationStorage()

    await Promise.all([first.initialize(), second.initialize()])

    expect(mockConnection.createConnection).toHaveBeenCalledTimes(1)
    expect(mockDatabase.open).toHaveBeenCalledTimes(1)
  })
