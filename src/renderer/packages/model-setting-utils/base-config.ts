...
export default abstract class BaseConfig implements ModelSettingUtil {
  ...
  private async listRemoteProviderModels(): Promise<ProviderModelInfo[]> {
    // The old implementation queried Chatbox's hosted model manifest. Keep
    // model discovery provider-local: use the user's saved models, the
    // provider's own `/models` endpoint, and the bundled registry fallback.
    return []
  }
