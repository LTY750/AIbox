...
describe('getContextMessageIds', () => {
  ...
  it('should apply maxCount after compaction filtering', () => {
    ...
    // With maxCount=2: summary is retained and the limit applies to recent
    // history plus the current input: [summary-1, m5, m6].
    const result = getContextMessageIds(session, 2)

    expect(result).toEqual(['summary-1', 'm5', 'm6'])
  })
  ...
  it('should retain system and summary IDs when limiting a compacted context', () => {
    const systemMessage = createMessage('sys', 'system', 'System prompt')
    const summary = createSummaryMessage('summary-1')
    const session: Session = {
      id: 'session-1',
      name: 'Test Session',
      messages: [
        ...
        createMessage('m1', 'user', 'Old'),
        createMessage('m2', 'assistant', 'Boundary'),
        createMessage('m3', 'user', 'Recent'),
        summary,
      ],
      compactionPoints: [createCompactionPoint('summary-1', 'm2', Date.now())],
    }

    expect(getContextMessageIds(session, 0)).toEqual(['sys', 'summary-1', 'm3'])
  })
})
