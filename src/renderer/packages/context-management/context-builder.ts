...
export function getContextMessageIds(session: Session, maxCount?: number): string[] {
  ...
  if (maxCount !== undefined) {
    // Keep the summary and system prompt outside the recent-history count. A
    // zero limit still retains the latest completed message for the token-count
    // UI, matching the send-context builder's current-input behavior.
    const head = contextMessages[0]?.role === 'system' ? contextMessages[0] : undefined
    const workingMessages = head ? contextMessages.slice(1) : contextMessages
    const summaryIds = workingMessages.filter((message) => message.isSummary).map((message) => message.id)
    const historyIds = workingMessages.filter((message) => !message.isSummary).map((message) => message.id)
    const effectiveLimit = Math.max(1, maxCount)
    const recentHistory = historyIds.slice(-effectiveLimit)
    return [...(head ? [head.id] : []), ...summaryIds, ...recentHistory]
  }
