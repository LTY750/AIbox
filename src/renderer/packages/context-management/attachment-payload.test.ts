...
describe('attachment-payload', () => {
  ...
  describe('selectMessagesForSendContext', () => {
    ...
    it('should preserve a compaction summary when limiting recent messages', () => {
      const messages: Message[] = [
        createMessage('user', 'old', 'old'),
        { ...createMessage('assistant', 'summary', 'summary'), isSummary: true },
        createMessage('user', 'current', 'current'),
      ]
      ...
      expect(result.map((m) => m.id)).toEqual(['summary', 'current'])
    })
