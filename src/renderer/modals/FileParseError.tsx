  ...
  const renderErrorTips = () => {
    ...
    if (isSessionAttachmentRagAuthError(errorCode)) {
      return (
        <Text>
          {t(
            'This large file needs a configured embedding model to finish indexing. Configure one, then retry this file. If you do not want to use indexed attachments, remove the file and upload a smaller attachment instead.'
          )}
        </Text>
      )
    }
