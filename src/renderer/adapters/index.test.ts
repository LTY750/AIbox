...
describe('createModel', () => {
  ...
  it('routes mobile SSE fetches through the native request client', async () => {
    const response = new Response('data: done\n\n')
    const apiRequestClient = {
    ...
    const result = await dependencies.request.fetchWithOptions('https://api.example.com/v1/chat/completions', {
      method: 'POST',
    ...
    expect(apiRequestClient.post).toHaveBeenCalledWith(
      'https://api.example.com/v1/chat/completions',
      { accept: 'text/event-stream', authorization: 'Bearer test' },
      JSON.stringify({ stream: true }),
      expect.objectContaining({ retry: 0, useProxy: true })
    )
  })

  it('routes mobile Gemini SSE URLs through the native request client', async () => {
    const apiRequestClient = {
    ...
    await dependencies.request.fetchWithOptions(
      'https://generativelanguage.googleapis.com/v1beta/models/gemini:streamGenerateContent?alt=sse',
      { method: 'POST', body: '{}' }
    )

    expect(apiRequestClient.post).toHaveBeenCalledOnce()
    expect(apiRequestClient.post).toHaveBeenCalledWith(
