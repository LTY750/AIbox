...
type MobileSettingsRecord = Record<string, any>

function extractSettingsSecrets(settings: MobileSettingsRecord): Record<string, unknown> {
  const secrets: Record<string, unknown> = {}
  const providers = settings.providers && typeof settings.providers === 'object' ? settings.providers : {}
  for (const [providerId, provider] of Object.entries(providers as Record<string, MobileSettingsRecord>)) {
    const secretFields = ['apiKey', 'accessKey', 'secretKey', 'sessionToken']
    ...
    for (const field of ['accessToken', 'refreshToken']) {
      if (typeof provider.oauth?.[field] === 'string' && provider.oauth[field]) {
        secrets[`providers.${providerId}.oauth.${field}`] = provider.oauth[field]
      }
    }
  }
  const parser = settings.extension?.documentParser
  for (const [section, fields] of Object.entries({
    ...
    for (const field of fields) {
      const value = parser?.[section]?.[field]
...
function removeSettingsSecrets(settings: MobileSettingsRecord): MobileSettingsRecord {
  const sanitized = structuredClone(settings)
  const providers = sanitized.providers && typeof sanitized.providers === 'object' ? sanitized.providers : {}
  for (const provider of Object.values(providers as Record<string, MobileSettingsRecord>)) {
    for (const field of ['apiKey', 'accessKey', 'secretKey', 'sessionToken']) delete provider[field]
    if (provider.oauth) {
      delete provider.oauth.accessToken
      delete provider.oauth.refreshToken
    }
  }
  const parser = sanitized.extension?.documentParser
  for (const section of ['llamaParse', 'mineru', 'textin']) {
    if (parser?.[section]) {
      for (const field of Object.keys(parser[section])) {
        if (['apiKey', 'apiToken', 'appId', 'secretCode'].includes(field)) delete parser[section][field]
      }
    }
  }
  return sanitized
...
function setPath(target: MobileSettingsRecord, path: string, value: unknown): void {
...
export default class MobilePlatform extends MobileSQLiteStorage implements Platform {
  ...
  public async getAllStoreValues(): Promise<{ [key: string]: any }> {
    const values = await super.getAllStoreValues()
    if (values.settings) values.settings = await this.getStoreValue('settings')
    return values
  }
