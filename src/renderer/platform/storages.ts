...
class SQLiteStorage {
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      this.database = await getSharedSQLiteDatabase('chatbox.db', 1)
    } catch (error) {
  ...
  async closeDatabase(): Promise<void> {
    // Do not close a shared native handle from an individual storage wrapper.
    // Other in-flight reads/writes may still be using it; keeping it open makes
    // close idempotent and removes the open/close race on Android.
    await this.ensureInitialized()
  }
}
