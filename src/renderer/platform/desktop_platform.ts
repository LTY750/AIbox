...
export default class DesktopPlatform implements Platform {
  ...
  async parseFileWithTextin(
    ...
    const filePath = this.getLocalFilePath(file)
    if (!filePath) {
      return { success: false, error: 'File path is required for TextIn parsing' }
    }

    return this.ipc.invoke('parser:parse-file-with-textin', {
      filePath,
      filename: file.name,
      mimeType: file.type,
  ...
  async cancelTextinParse(filePath: string): Promise<{ success: boolean; error?: string }> {
    return this.ipc.invoke('parser:cancel-textin-parse', filePath)
  }
