...
export interface Platform extends Storage {
  ...
  /**
   * Receives native application activity changes when the platform can report
   * them. Mobile uses Capacitor's Activity lifecycle; other platforms do not
   * need to implement this lower-level signal.
   */
  ...
  // Parse file using TextIn XParse (Desktop and mobile)
  parseFileWithTextin?(
    file: File,
    credentials: { appId: string; secretCode: string }
  ): Promise<{ success: boolean; content?: string; error?: string; cancelled?: boolean }>

  // Cancel TextIn parsing task (Desktop only; mobile requests are native HTTP calls)
  cancelTextinParse?(filePath: string): Promise<{ success: boolean; error?: string }>
