...
export function Artifact(props: { htmlCode: string; previewUrl?: string; reloadSign?: number; className?: string }) {
  ...
  const sendIframeMsg = useCallback((type: 'html', code: string) => {
    ...
    // postMessage can only target it with "*". The Window reference is taken
    // directly from our fixed iframe rather than discovered by origin.
  ...
  return (
    <iframe
      ...
      // Keep every generated artifact in an opaque origin. In particular, do
      // not grant allow-same-origin: a hostile HTML artifact must not be able
      // to reach the host WebView or its Capacitor bridge.
