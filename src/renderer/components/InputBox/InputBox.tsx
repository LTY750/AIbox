...
const InputBox = forwardRef<InputBoxRef, InputBoxProps>(
  (
    ...
    useEffect(() => {
      if (!featureFlags.skills || skillCommandQuery === null || inputSkills.length > 0 || inputSkillsLoading) {
        return
      }
    ...
    const skillMenuOpen =
      featureFlags.skills && skillCommandQuery !== null && matchingInputSkills.length > 0 && !isAwaitingToolApproval
                  ...
                  return (
                    <FileMiniCard
                      ...
                      onDelete={() => {
                        ...
                        // Cancel any ongoing cloud parsing for this file
                        ...
                        if (filePath && platform.cancelTextinParse) {
                          platform.cancelTextinParse(filePath).catch(() => {
                            // Ignore cancellation errors
                          })
                        }
            ...
            <Flex align="center" gap={0} className="shrink-0 w-full" justify="space-between">
              ...
              <Flex align="center" gap={0}>
                <AttachmentMenu
                  ...
                  onCameraCaptureClick={platform.type === 'mobile' ? onCameraCaptureClick : undefined}
...
const AttachmentMenu: React.FC<{
  ...
  t: (key: string) => string
}> = ({ onImageUploadClick, onCameraCaptureClick, onFileUploadClick, t }) => {
  const isSmallScreen = useIsSmallScreen()
  const toolbarIconSize = isSmallScreen ? 22 : 18
  return (
    <Menu
    ...
    >
      ...
      <Menu.Dropdown>
        ...
        {onCameraCaptureClick && (
          <Menu.Item leftSection={<IconPhoto size={16} />} onClick={onCameraCaptureClick}>
            {t('Take Photo')}
          </Menu.Item>
        )}
        <Menu.Item
