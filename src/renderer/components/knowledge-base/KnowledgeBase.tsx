...
const KnowledgeBasePage: React.FC = () => {
  ...
  const globalDocumentParser = useSettingsStore((state) => state.extension?.documentParser)
  ...
  const [newDocumentParser, setNewDocumentParser] = useState<DocumentParserConfig>({ type: 'llamaparse' })
  ...
  const createKb = async () => {
    ...
    const documentParser = (() => {
      if (newDocumentParser.type === 'llamaparse' && !newDocumentParser.llamaParse?.apiKey) {
        return { ...newDocumentParser, llamaParse: globalDocumentParser?.llamaParse }
      }
      if (
        newDocumentParser.type === 'textin' &&
    ...
    try {
      ...
      setNewDocumentParser({ type: 'llamaparse' })
      setShowCreate(false)
      await fetchKbList()
    } catch (e) {
