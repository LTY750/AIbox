...
/**
 * Parse an external parser error response into a user-friendly message.
 */
...
export function RemoteRetryModal({ opened, onClose, failedFiles, onSuccess }: RemoteRetryModalProps) {
  ...
  // Filter files that failed locally and can be retried with the configured cloud parser.
  ...
  const handleRetry = async (fileId: number, filename: string) => {
    ...
    try {
      ...
      await controller.retryFile(fileId, true)
      toast.success(t('File {{filename}} queued for cloud parsing', { filename }))
      onSuccess()
    } catch (error) {
  ...
  const handleRetryAll = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success(t('{{count}} file(s) queued for cloud parsing', { count: successCount }))
      }
      if (failCount > 0) {
  ...
  return (
    <Modal opened={opened} onClose={onClose} title={t('Retry with cloud parser')} size="lg" centered>
      <Stack gap="md">
        {/* Warning alert */}
        <Alert color="yellow" icon={<IconAlertTriangle size={16} />}>
          {t('Cloud parsing sends files to the configured parser service. Please be cautious with sensitive files.')}
        </Alert>
