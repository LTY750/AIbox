import { Button, Group, Input, PasswordInput, Select, Stack, Text, TextInput } from '@mantine/core'
...
const PARSER_OPTIONS: { value: DocumentParserType; label: string; description: string }[] = [
  ...
  },
  {
    value: 'llamaparse',
    label: 'LlamaParse',
    description: 'Tries local parsing first, then uses LlamaParse for formats the device cannot read.',
  },
  {
    value: 'mineru',
    label: 'MinerU',
...
export const DocumentParserSelector: React.FC<DocumentParserSelectorProps> = ({
  ...
  const handleTextinAppIdChange = useCallback(
    (value: string) => {
      setTextinAppId(value)
      onParserConfigChange({
        type: 'textin',
        textin: { appId: value, secretCode: textinSecretCode },
      })
    },
    [onParserConfigChange, textinSecretCode]
  )

  const handleTextinSecretCodeChange = useCallback(
    (value: string) => {
      setTextinSecretCode(value)
      onParserConfigChange({
        type: 'textin',
        textin: { appId: textinAppId, secretCode: value },
      })
    },
  ...
  return (
    <Stack gap="xs">
      ...
      {parserConfig.type === 'textin' && !disabled && (
        <Stack gap="xs">
          <TextInput
            label={t('TextIn App ID')}
            value={textinAppId}
            onChange={(e) => handleTextinAppIdChange(e.currentTarget.value)}
          />
          <PasswordInput
