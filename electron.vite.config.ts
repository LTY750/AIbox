...
/**
 * Vite plugin to inject the release date without relaxing the page CSP.
 */
  ...
  return {
    ...
    transformIndexHtml() {
      return [
        {
          tag: 'meta',
          attrs: { name: 'chatbox-release-date', content: releaseDate },
          injectTo: 'head',
        },
      ]
    },
...
export function excludeMobileProductionDevRoutes(): Plugin {
  const routeIds = new Map([
    ['/src/renderer/routes/dev/context-generator.tsx', '/dev/context-generator'],
    ['/src/renderer/routes/dev/ui-inventory.tsx', '/dev/ui-inventory'],
  ])
  return {
    name: 'exclude-mobile-production-dev-routes',
    enforce: 'pre',
    load(id) {
      const normalizedId = id.split(path.sep).join('/')
      const entry = [...routeIds.entries()].find(([file]) => normalizedId.endsWith(file))
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    renderer: {
      ...
      plugins: [
        isProduction && isMobile ? excludeMobileProductionDevRoutes() : undefined,
        TanStackRouterVite({
          target: 'react',
